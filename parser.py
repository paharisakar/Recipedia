...
def all_links(s):
    ...
    while unvisited != set():
        ...
        p_text = requests.get(p).text
        # p_links = re.findall(r'<a href="(.*?recipe/)" ', p_text) # `p_links` stores all the links found on the page `p`
        # p_links = re.findall(r'<a href="(https://www.allrecipes.com/recipe/.+)" data-internal', p_text)
        # p_links = re.findall(r'<a href="https://www.allrecipes.com/recipe/(.+?)=recipe"', p_text)
        print(p_text)
        p_links = re.findall(r'<a href="(https://www.allrecipes.com/recipe/.+?=recipe)"', p_text)
        print(p_links)
...
my_dict = all_links('https://www.allrecipes.com/')
type(my_dict)
